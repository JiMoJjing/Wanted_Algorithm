...
void RedBlackTree::RestructureAfterInsert(Node* newNode)
{
	...
	while (newNode != root && newNode->Parent()->GetColor() == Color::Red)
	{
		...
		if (newNode->Parent() == newNode->Parent()->Parent()->Left())
		{
			...
			// Case2 / Case3.
			// Case2: 지그재그 형태 부모의 위치와 나의 위치가 반대(일직선으로 변경 후 Case3 처리).
		...
		else // 삼촌이 왼쪽일 경우.
		{
			...
			// 해결: 부모를 Black, 할아버지를 Red로 변경 후, 할아버지를 기준으로 회전.
			// Case2 / Case3.
			// Case2: 지그재그 형태 부모의 취기와 나의 위치가 반대(일직선으로 변경 후 Case3 처리).
...
void RedBlackTree::RestructureAfterRemove(Node* node)
{
	...
	while (node->Parent() != nullptr && node->GetColor() == Color::Black)
	{
		...
		if (node == node->Parent()->Left())
		{
			...
			// Case3: 형제노드는 검정, 형제노드의 왼쪽 자손이 빨간색.
			...
			// Case4: 형제노드는 검정, 형제노드의 오른쪽 자손이 빨간색.
		...
		// Case3: 형제노드는 검정, 형제노드의 오른쪽 자손이 빨간색.
		// 해결: 형제의 오른쪽 자식을 Black으로 변경, 형제를 Red로 변경 후 형제 기준 우회전.
		if (sibling->Right()->GetColor() == Color::Red)
		{
			sibling->Right()->SetColor(Color::Black);
			sibling->SetColor(Color::Red);
		...
		// Case4: 형제노드는 검정, 형제노드의 왼쪽 자손이 빨간색.
		// 해결: 형제를 부모와 같은 색으로 설정, 부모를 Black으로 변경.
...
void RedBlackTree::RemoveImpl(Node* node)
{
	...
	else
	{
		...
		trail = FindMaxRecursive(removed->Left());

		// 현재 노드의 값을 대상 노드의 값으로 변경.
		removed->SetData(trail->Data());

		trail->Parent()->SetRight(nil);
...
void RedBlackTree::DestoryRecursive(Node* node)
{
	if (node == nil)
	{
		return;
	...
	// 왼쪽 / 오른쪽 자손.
	DestoryRecursive(node->Left());
	DestoryRecursive(node->Right());
